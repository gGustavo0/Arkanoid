...
void Arkanoid::createBlocks() {
	...
	blocks.resize(14);
	for (int i = 0; i < 14; i++)
		for (int j = 0; j < 4; j++) blocks[i].push_back(*new Block(50 + 50 * i, 50 + 50 * j, bonuses, 50));
}
	...
	while (window->isOpen())
	{
		...
		while (window->pollEvent(event))
		{
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)){ 
				...
				for (auto& ball : balls) ball.moveWithPlatform(platform.platform);
			}
		...
		if (counter.points >= 10) { window->clear(sf::Color::Black); window->draw(counter.getWin());  }
		else {
			if (counter.ifGameOver()) { window->clear(sf::Color::Black); window->draw(counter.getGameOver()); cout << counter.points << endl; }
			else {
				for (auto& ball : balls) {
				...
				if (counter.points == 0)
				{
					...
					if (balls.size() == 0) balls.push_back(*new Ball(platform.platform.getPosition().x, platform.platform.getPosition().y));
				...
				for (auto& i : blocks)
					for (auto& j : i)
						if (j.exists()) window->draw(j.block);
						else {
							//remove
						}
